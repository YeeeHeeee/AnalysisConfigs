...
def stitching_func(params, metadata, events, size, shape_variations):
  if metadata["year"] == "2016_PreVFP" and metadata["sample"] == "TTToSemiLeptonic":
    return np.select(
      condlist=[
        (events.GenTT.mass>=0.0) & (events.GenTT.mass<700.0) & ((events.count_l==1) & (events.count_nu==1)),
        (events.GenTT.mass>=700.0) & (events.GenTT.mass<1000.0) & ((events.count_l==1) & (events.count_nu==1)),
        (events.GenTT.mass>=1000.0) & (events.GenTT.mass<5000.0) & ((events.count_l==1) & (events.count_nu==1)),
